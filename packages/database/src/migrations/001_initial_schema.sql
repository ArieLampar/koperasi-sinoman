-- Migration: Initial Schema for Koperasi Sinoman Phase 1
-- Created: 2024-09-29T10:00:00.000Z
-- Description: Create all Phase 1 tables, indexes, and Row Level Security policies for member registration, savings accounts, and transaction logging
...
DECLARE
    ...
    final_code TEXT;
BEGIN
    -- Generate base code from first name and year
    base_code := UPPER(LEFT(SPLIT_PART(NEW.full_name, ' ', 1), 4)) ||
    ...
    LOOP
        ...
        IF NOT EXISTS (SELECT 1 FROM members WHERE referral_code = final_code) THEN
            EXIT;
        END IF;
...
-- Member registration workflow function
CREATE OR REPLACE FUNCTION create_member_with_mandatory_accounts(
...
BEGIN
    -- Insert new member
    INSERT INTO members (
        full_name, nik, email, phone, date_of_birth, gender,
        address, village, district, city, province, postal_code,
        occupation, monthly_income, referred_by,
        membership_type, membership_status
    ) VALUES (
        p_full_name, p_nik, p_email, p_phone, p_date_of_birth, p_gender,
        p_address, p_village, p_district, p_city, p_province, p_postal_code,
        p_occupation, p_monthly_income, p_referred_by,
        'regular', 'pending'
    ) RETURNING id INTO new_member_id;

    -- Get mandatory savings types
    SELECT id INTO pokok_type_id FROM savings_types WHERE code = 'POKOK';
    SELECT id INTO wajib_type_id FROM savings_types WHERE code = 'WAJIB';

    -- Create mandatory savings accounts
    IF pokok_type_id IS NOT NULL THEN
        INSERT INTO savings_accounts (member_id, savings_type_id, balance, status)
        VALUES (new_member_id, pokok_type_id, 0, 'active');
    END IF;

    IF wajib_type_id IS NOT NULL THEN
        INSERT INTO savings_accounts (member_id, savings_type_id, balance, status)
        VALUES (new_member_id, wajib_type_id, 0, 'active');
    END IF;

    -- Create referral record if referred
    IF p_referred_by IS NOT NULL THEN
        INSERT INTO referrals (referrer_id, referred_id, referral_code, status)
        SELECT p_referred_by, new_member_id, m.referral_code, 'pending'
        FROM members m WHERE m.id = p_referred_by;
    END IF;
...
CREATE TRIGGER tr_members_generate_referral_code
    BEFORE INSERT ON members
    FOR EACH ROW
...
-- Note: This section would be executed if we need to rollback this migration
-- It should be in reverse order of creation

/*
-- Drop all triggers first
DROP TRIGGER IF EXISTS tr_admin_users_audit ON admin_users;
DROP TRIGGER IF EXISTS tr_transactions_audit ON transactions;
DROP TRIGGER IF EXISTS tr_savings_accounts_audit ON savings_accounts;
DROP TRIGGER IF EXISTS tr_members_audit ON members;

DROP TRIGGER IF EXISTS tr_transactions_generate_number ON transactions;
DROP TRIGGER IF EXISTS tr_savings_accounts_generate_number ON savings_accounts;
DROP TRIGGER IF EXISTS tr_members_generate_referral_code ON members;
DROP TRIGGER IF EXISTS tr_members_generate_number ON members;

DROP TRIGGER IF EXISTS tr_admin_users_updated_at ON admin_users;
...
DROP TRIGGER IF EXISTS tr_savings_accounts_updated_at ON savings_accounts;
DROP TRIGGER IF EXISTS tr_savings_types_updated_at ON savings_types;
DROP TRIGGER IF EXISTS tr_kyc_documents_updated_at ON kyc_documents;
DROP TRIGGER IF EXISTS tr_members_updated_at ON members;

-- Drop all functions
DROP FUNCTION IF EXISTS create_member_with_mandatory_accounts;
DROP FUNCTION IF EXISTS audit_trigger_function;
DROP FUNCTION IF EXISTS generate_referral_code;
DROP FUNCTION IF EXISTS generate_transaction_number;
DROP FUNCTION IF EXISTS generate_account_number;
DROP FUNCTION IF EXISTS generate_member_number;
...
DROP TYPE IF EXISTS savings_account_status;
DROP TYPE IF EXISTS savings_type_code;
DROP TYPE IF EXISTS kyc_status;
DROP TYPE IF EXISTS membership_status;
DROP TYPE IF EXISTS membership_type;
DROP TYPE IF EXISTS gender_type;

-- Drop extensions (only if not used by other objects)
